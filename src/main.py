...
def generateDefaultSettingConfig():
    ...
    settingSaver(default_theme, default_appTransparency, default_highlightColor,
                 default_highlightOption, default_scanMethod, default_language)
...
def checkConfigExist():
    ...
    try:
        ...
        # if there are no config files, create new files with default setting
        messagebox.showwarning(
            title="Warning!", message="No config properties file found!\nPlease try to restart the application or contact administrator.")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        try:
            ...
            messagebox.showwarning(
                title="Warning!", message="Error getting application's size: " + str(e) + "!!!\nUsing default size instead.")
        ...
        sideBarButtonFont = ctk.CTkFont(
            family="Century Gothic", size=17, weight='bold')
        header_1Font = ctk.CTkFont(
            family="Century Gothic", size=16, weight='bold')
        ...
        self.logo_image = ctk.CTkImage(Image.open(
            os.path.join(image_path, "logo.png")), size=(60, 60))
        ...
        self.navigation_frame_label = ctk.CTkLabel(self.navigation_frame, text="myShortcut", image=self.logo_image,
                                                   compound="left", font=ctk.CTkFont(family="Century Gothic", size=20, weight="bold"))
        ...
        self.textCompare_frame = ctk.CTkFrame(
        ...
        self.textCompare_frame_textbox1 = ctk.CTkTextbox(
            self.textCompare_frame)
        self.textCompare_frame_textbox1.configure(
            autoseparators=True, undo=True, maxundo=-1, font=defaultTextBoxFont)
        self.textCompare_frame_textbox1.grid(
            row=0, column=0, padx=20, pady=(20, 0), sticky="nsew")
        self.textCompare_frame_textbox2 = ctk.CTkTextbox(
            self.textCompare_frame)
        self.textCompare_frame_textbox2.configure(
            autoseparators=True, undo=True, maxundo=-1, font=defaultTextBoxFont)
        self.textCompare_frame_textbox2.grid(
            row=0, column=1, padx=20, pady=(20, 0), sticky="nsew")

        # Other Widgets
        self.textCompare_frame_button = ctk.CTkButton(
            self.textCompare_frame, text="Compare Text", font=defaultTextButtonFont, command=lambda: get_textCompare())
        self.textCompare_frame_button.grid(
            row=1, column=1, padx=20, pady=20, sticky="ew")

        self.optionText_comboBox_var = ctk.StringVar(
            value="Select Text Options")
        ...
        self.optionText_comboBox = ctk.CTkComboBox(master=self.textCompare_frame, font=defaultTextButtonFont, state="readonly",
                                                   values=["Remove <xyz> tags", "Remove 'xyz>' tag only",
                                                           "Remove extra spaces", "Change to uppercase", "Change to Lowercase"],
                                                   command=combobox_callback, variable=self.optionText_comboBox_var)
        self.optionText_comboBox.grid(
        ...
        def processText(option):
            ...
            if (option == 1):
                ...
                for sentences in pT.processText_removeTag(text1):
                    self.textCompare_frame_textbox1.insert(
                        "end", sentences + '\n')
                for sentences in pT.processText_removeTag(text2):
                    self.textCompare_frame_textbox2.insert(
                        "end", sentences + '\n')
            elif (option == 2):
        ...
        def get_textCompare():
            ...
            if (len(diff_all) == 0):
                messagebox.showinfo(title="Comparison Result",
                                    message="Both texts are identical!")
            else:
                if (highlightOption.lower() == 'highlight text' or highlightOption.lower() == 'highlight line'):
                    dmp = dmp_module.diff_match_patch()
                    diff = dmp.diff_main(text1, text2)
                    dmp.diff_cleanupSemantic(diff)
                    self.result_window = cRW.ComparisonTopWindow(
                        diff, highlightColor, highlightOption.lower())
                else:
                    messagebox.showerror(
                        title="Error", message="Highlight Option setting is not corrected.\nPlease check your settings!")
        ...
        self.setting_frame = ctk.CTkScrollableFrame(
            self, corner_radius=0, fg_color="transparent")
        ...
        self.theme_setting_menu = ctk.CTkOptionMenu(
            self.setting_frame, values=theme_values, font=defaultTextButtonFont, command=self.theme_setting_event)
        self.theme_setting_menu.grid(
            row=1, column=2, padx=0, pady=10, sticky="ew")
        ...
        try:
            ...
            messagebox.showerror(title="Error", message="Error occurs: " + str(
        ...
        self.highlight_color_entry = ctk.CTkEntry(self.setting_frame, placeholder_text=highlightColor, validate="key", validatecommand=(
            self.register(highlight_color_validate_input), '%P'), font=defaultTextButtonFont, corner_radius=5)
        self.highlight_color_entry.grid(
            row=4, column=2, padx=0, pady=10, sticky="ew")
        ...
        self.highlight_option_settings_switch = ctk.CTkOptionMenu(
            self.setting_frame, variable=highlight_option_var, values=highlight_option_settings_value, font=defaultTextButtonFont)
        self.highlight_option_settings_switch.grid(
            row=5, column=2, sticky="nsew", padx=0, pady=10)
        ...
        self.OCR_method_setting_switch = ctk.CTkOptionMenu(
            self.setting_frame, variable=OCR_method_var, values=OCR_method_setting_values, font=defaultTextButtonFont)
        self.OCR_method_setting_switch.grid(
            row=7, column=2, sticky="nsew", padx=0, pady=10)
        ...
        self.OCR_language_setting_switch = ctk.CTkOptionMenu(
            self.setting_frame, variable=OCR_language_var, values=OCR_language_setting_switch_values, font=defaultTextButtonFont)
        self.OCR_language_setting_switch.grid(
            row=8, column=2, sticky="nsew", padx=0, pady=10)
        ...
        self.save_setting_btn = ctk.CTkButton(
            self.setting_frame, text="Save", font=defaultTextButtonFont, command=lambda: saveSetting())
        self.save_setting_btn.grid(
            row=18, column=3, padx=(0, 30), pady=10, sticky="nse")
        ...
        def saveSetting():
            ...
            new_highlightColor = highlightColor if get_new_highlightColor in [
                None, ""] else get_new_highlightColor
            ...
            if re.match(hex_color_pattern, new_highlightColor):
                try:
                    settingSaver(new_theme, new_appTransparency, new_highlightColor,
                                 new_highlightOption, new_scanText_method, new_scanText_language)
                    propertiesSaver(self.winfo_width(), self.winfo_height())
                    messagebox.showinfo(
                        title="Restart required", message="Saved successfully.\nThe app will now restart.")
                    # Restarts the current program.
                    python = sys.executable
                    os.execl(python, python, * sys.argv)
                except Exception as e:
                    messagebox.showerror(
                        title="Error", message="Unknown error occurs!\n" + str(e))
            else:
                messagebox.showerror(
                    title="Error", message="The highlight color is incorrect. Please make sure it is in the correct HEX format.")
        ...
        self.check_OCR_lang_btn = ctk.CTkButton(
            self.setting_frame, text="Check language", font=defaultTextButtonFont, command=lambda: check_available_OCR_language())
        self.check_OCR_lang_btn.grid(
            row=10, column=2, sticky="nsew", padx=0, pady=10)

        self.add_OCR_lang_btn = ctk.CTkButton(
            self.setting_frame, text="Add", font=defaultTextButtonFont, command=lambda: add_OCR_language())
        self.add_OCR_lang_btn.grid(
            row=11, column=3, sticky="nsew", padx=60, pady=10)

        # OptionMenu
        OCR_lang_values = ["English (Global)", "English (US)", "Korean", "Japanese", "Chinese (Simplified)",
        ...
        self.OCR_lang_optionmenu = ctk.CTkOptionMenu(
            self.setting_frame, values=OCR_lang_values, font=defaultTextButtonFont)
        self.OCR_lang_optionmenu.grid(
            row=11, column=2, sticky="nsew", padx=0, pady=10)

        def check_available_OCR_language():
            output = ""
            for language in OCR_lang_values:
                if language in OCR_lang_dict:
                    result, powershell_result = oLC.is_ocr_language_installed(
                        OCR_lang_dict[language])
                    if (result):
        ...
        def add_OCR_language():
            ...
            if (OCR_lang_select_value == ""):
                messagebox.showerror(
                    title="Add OCR Language", message="You have not selected any language.")
            elif OCR_lang_select_value in OCR_lang_values:
                result, powershell_result = oLC.ocr_lang_installer(
                    OCR_lang_dict[OCR_lang_select_value])
                if (result):
                    message = f"{OCR_lang_select_value} is installed successfully."
                    messagebox.showinfo(
                        title="Add OCR Language", message=message)
                else:
                    message = f"Failed to install {OCR_lang_select_value} OCR language.\nPlease try again or contact administrator."
                    messagebox.showerror(
                        title="Add OCR Language", message=message)
                    messagebox.showerror(
                        title="Add OCR Language", message=powershell_result)
            else:
                messagebox.showerror(
                    title="Add OCR Language", message="The selected language is not available yet.")
    ...
    def select_frame_by_name(self, name):
        ...
        self.textCompare_button.configure(
            fg_color=("gray75", "gray25") if name == "textCompare" else "transparent")
